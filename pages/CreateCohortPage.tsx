...
import { ArrowLeft, Plus, X, Sparkles, ChevronDown } from 'lucide-react';
...
interface ObjectiveEntry {
  id: string;
  name: string;
  objectiveId?: string; // ID from API if created
}
...
export function CreateCohortPage() {
  ...
  const [objectives, setObjectives] = useState<ObjectiveEntry[]>([{ id: '1', name: '' }]);
  ...
  const handleAddObjective = () => {
    setObjectives([...objectives, { id: Date.now().toString(), name: '' }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create objectives that don't have IDs yet
    const objectiveIds: string[] = [];
    for (const obj of objectives) {
      if (!obj.name.trim()) continue;

      if (obj.objectiveId) {
        // Already created, use existing ID
        objectiveIds.push(obj.objectiveId);
      } else {
        ...
        try {
          ...
          objectiveIds.push(newObjective._id);
        } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-3">
                What goals do you want students to achieve? (Objectives will be created when you submit)
              </p>
