...
export function CreateCohortPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('clarivue_custom_tags');
    if (stored) {
      try {
  ...
  const handleAddCustomTag = () => {
    if (newTag.trim() && !customTags.includes(newTag.trim())) {
      ...
      setCustomTags([...customTags, newTagValue]);
      setNewTag('');
      
      // Save to localStorage if not already saved
      if (!savedTags.includes(newTagValue)) {
        const updatedSavedTags = [...savedTags, newTagValue];
        setSavedTags(updatedSavedTags);
        localStorage.setItem('clarivue_custom_tags', JSON.stringify(updatedSavedTags));
      }
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <CreatableSelect
                  ...
                  placeholder="Select or create term..."
                  storageKey="clarivue_terms"
                />
              </div>
              ...
              <div>
                ...
                <CreatableSelect
                  value={program}
                  onChange={setProgram}
                  options={['Computer Science', 'Data Science', 'Engineering', 'Business', 'Design']}
                  placeholder="Select or create program..."
                  storageKey="clarivue_programs"
                />
              </div>
            </div>
            ...
            <div>
              ...
              {/* Show saved tags that can be selected */}
              {savedTags.length > 0 && (
                <div className="mb-3">
                  <p className="text-xs text-gray-500 mb-2">Previously used tags (click to add):</p>
